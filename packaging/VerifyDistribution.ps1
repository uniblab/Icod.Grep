...
try {
    ...
    $toolShim = Get-ExecutablePath -Directory $toolPath -CommandName 'grep'
    Invoke-Tool -Path $toolShim -Arguments @('--version')

    $samplePath = Join-Path $validationRoot 'sample.txt'
    [System.IO.File]::WriteAllText(
        $samplePath,
        "alpha`nbeta`ngamma`n",
        [System.Text.UTF8Encoding]::new($false)
    )
    Invoke-Tool -Path $toolShim -Arguments @('-n', 'beta', $samplePath)
